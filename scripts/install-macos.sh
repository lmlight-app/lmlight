#!/bin/bash
# LM Light インストーラー for macOS
# 使い方: curl -fsSL https://raw.githubusercontent.com/lmlight-app/lmlight/main/scripts/install-macos.sh | bash
...
echo ""
echo -e "${BLUE}╔═══════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║      LM Light インストーラー for macOS                ║${NC}"
echo -e "${BLUE}╚═══════════════════════════════════════════════════════╝${NC}"
echo ""
...
if command -v psql &>/dev/null; then
    success "PostgreSQL が見つかりました"
else
    warn "PostgreSQL が見つかりません"
...
# ============================================================
info "ステップ 3/5: PostgreSQL をセットアップ中..."

if command -v psql &>/dev/null; then
    # PostgreSQL が動いているか確認
    if ! pg_isready -q 2>/dev/null; then
        warn "PostgreSQL が起動していません。起動してください: brew services start postgresql@16"
    else
        # データベースとユーザー作成
        info "データベースを作成中..."
...
SQLEOF
    ...
    warn "PostgreSQL がインストールされていないため、データベースセットアップをスキップしました"
fi

# ============================================================
# ステップ 4: Ollama セットアップ
# ============================================================
info "ステップ 4/5: Ollama をセットアップ中..."
...
if command -v ollama &>/dev/null; then
    ...
    for model in "${MODELS[@]}"; do
        if ollama list 2>/dev/null | grep -q "$model"; then
            ...
            info "$model をダウンロード中..."
            ollama pull "$model" || warn "$model のダウンロードに失敗しました"
        fi
    done
else
    warn "Ollama がインストールされていないため、モデルダウンロードをスキップしました"
fi

# ============================================================
# ステップ 5: 設定とスクリプト作成
# ============================================================
info "ステップ 5/5: 設定を作成中..."
